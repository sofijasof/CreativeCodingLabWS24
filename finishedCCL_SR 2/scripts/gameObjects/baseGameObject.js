...
class BaseGameObject {
    ...
    getBoxBounds = function () {
        let widthReduced = this.width*0.6; //% of the width
        let bounds = {
            left: this.x + widthReduced,
            right: this.x + this.width - widthReduced,
            top: this.y,
    ...
    applyGravity = function () {
        ...
        const newY = this.y + (this.physicsData.fallVelocity * global.deltaTime + this.physicsData.prevFallingVelocity) / 2;
    
        // Prevent falling out of the canvas
        if (newY + this.height > global.canvas.height) {
        ...
        for (let i = 0; i < global.allGameObjects.length; i++) {
            ...
            if (otherObject.active && otherObject.blockGravityForces) {
                const collisionHappened = global.detectBoxCollision(this, otherObject);
                if (collisionHappened) {
                    if (this.physicsData.fallVelocity > 0) {
                        this.physicsData.isGrounded = true;
                        this.y = otherObject.getBoxBounds().top - this.height;
                    } else if (this.physicsData.fallVelocity < 0) {
                        this.y = otherObject.getBoxBounds().bottom;
                    }
                    this.physicsData.jumpForce = 0;
                    this.physicsData.fallVelocity = 0;
                }
